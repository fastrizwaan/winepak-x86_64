...
if   [ "${RUNNER}" = "proton" ]; then
       if [ -d "${RUNNERS_DIR}/${PROTON_DIR}" ]; then
            echo "${RUNNER} Directory already exists, not downloding"
       else
...
# Check if user given runner exists as a directory in runners/
if ! [ -d "${RUNNERS_DIR}/${RUNNER}" ] ; then
       echo "Sorry ${RUNNERS_DIR}/${RUNNER} not found"
       exit 1
fi
...
NICE_NAME_NOSPACE="$(echo ${NICE_NAME}|tr ' ' '_')"
...
export LD_LIBRARY_PATH=$(pwd):$LD_LIBRARY_PATH:/app/lib:/app/lib32:/app/$NAME:/app/lib/wine:/app/lib32/wine:/app/lib/i386-linux-gnu:/app/lib/debug/lib/i386-linux-gnu:/app/lib64:/app/lib/i386-linux-gnu:/app/lib/wine:/app/lib64/wine
...
if [ -e winepak.x86_64.md ] ; then
	cat winepak.x86_64.md | sed -e "s|FLATPAKNAME|org.winepak.x86_64.${NICE_NAME_NOSPACE}|" -e "s|WINEPREFIX|\$WINEPREFIX|"
fi
...
#	   touch ~/.local/share/winepak.x86_64/$NAME/dxsetup-done.txt
#        fi
if ! [ -f ~/.local/share/winepak.x86_64/$NAME/firstrun ]; then
 WINEDLLOVERRIDES="mscoree=d;mshtml=d;" wineboot -u ;
 unset WINEDLLOVERRIDES
 touch ~/.local/share/winepak.x86_64/$NAME/firstrun
fi
...
[Application]
name=org.winepak.x86_64.${NICE_NAME_NOSPACE}
...
[Environment]
LD_LIBRARY_PATH=/app/lib:/app/lib:/app/lib32:/app/lib64:/app/lib/i386-linux-gnu:/app/lib/wine:/app/lib64/wine:$(pwd)
WINEDLLPATH=/app/dlls/lib32:/app/dlls/lib:/app/lib32/wine/wined3d:/app/lib/wine/wined3d
...
flatpak --user install -y --app --bundle "\$DIR/org.winepak.x86_64.${NICE_NAME_NOSPACE}.flatpak" || echo "Installation failed. Check if you have Flatpak properly configured. See http://flatpak.org/ for more info."
EOF
...
set -ex
flatpak --user uninstall org.winepak.x86_64.${NICE_NAME_NOSPACE}
EOF
...
#!/bin/bash
set -ex
flatpak run org.winepak.x86_64.${NICE_NAME_NOSPACE} \$@
EOF

echo -n "8.  Creating desktop shortcut   "
cat << EOF >"${TARGET_DIR}/package/export/share/applications/org.winepak.x86_64.${NICE_NAME_NOSPACE}.desktop" || exit 1 && echo -e "\t[x]"
[Desktop Entry]
Version=1.0
Name=$NICE_NAME
Exec=run.sh
Icon=org.winepak.x86_64.${NICE_NAME_NOSPACE}
StartupNotify=true
Terminal=false
Type=Application
Categories=Game;
...
hicolor/128x128/apps/org.winepak.x86_64.${NICE_NAME_NOSPACE}.png"
...
icotool -x "${TEMPDIR}/${EXE}.ico" -o ${TEMPDIR} 2>/dev/null

#echo "Icotool: Extracting png files from ${EXE}.ico" 
BEST_PNG="$(ls -S -1 ${TEMPDIR}/*.png | head -n 1)"

cp  "${BEST_PNG}" "${ICONFILE}" && (echo -e "\t[x]" && WARN="false") || exit 1
...
flatpak build-bundle --arch=$ARCH ${TARGET_DIR}/repo  "${TARGET_DIR}/[winepak.x86_64]$DOT_NAME/org.winepak.x86_64.${NICE_NAME_NOSPACE}.flatpak" org.winepak.x86_64.${NICE_NAME_NOSPACE} || exit 1 && (echo -e "\t[x]")

echo "20. Flatpak created! 			             [x]"

# removed runtime-repo for offline installation
#flatpak build-bundle --arch=$ARCH ${TARGET_DIR}/repo  "${TARGET_DIR}/[winepak.x86_64]$DOT_NAME/org.winepak.x86_64.${NICE_NAME_NOSPACE}.flatpak org.winepak.x86_64.${NICE_NAME_NOSPACE}" --runtime-repo="https://flathub.org/repo/flathub.flatpakrepo"
