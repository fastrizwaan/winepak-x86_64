...
# make_winepak32.sh AppName AppDirectory/ executable.exe # exe from AppDirectory/App.exe
...
mkdir -p target/\[winepak32\]$DOT_NAME
...
export WINEPREFIX=~/.local/share/winepak32/$NAME/
...
if [ -e winepak32.md ] ; then
	cat winepak32.md | sed -e "s|FLATPAKNAME|org.winepak32.$NAME|" -e "s|WINEPREFIX|\$WINEPREFIX|"
fi

if [ -e winepak32.reg ] ; then
	$WINEEXE regedit /C winepak32.reg
fi
...
#
#	touch ~/.local/share/winepak32/$NAME/dxsetup-done.txt
else
# Enable the lines below if dxsetup openal and vcredist is required or 
# run ./run.sh dxsetup
#	if [ ! -f ~/.local/share/winepak32/$NAME/dxsetup-done.txt ];
#	   then
#           $WINEEXE /app/$APP/_CommonRedist/DirectX/Jun2010/DXSETUP.exe /SILENT
#	   $WINEEXE /app/$APP/_CommonRedist/vcredist/2010/vcredist_x86.exe /SILENT
#	   $WINEEXE /app/$APP/_CommonRedist/vcredist/2012/vcredist_x86.exe /SILENT
...
cat << EOF >target/\[winepak32\]$DOT_NAME/install.sh
#!/bin/bash
...
cat << EOF >target/\[winepak32\]$DOT_NAME/uninstall.sh
#!/bin/bash
...
#!/bin/bash
set -ex
flatpak run org.winepak32.$NAME \$@
EOF

echo "8.  Creating desktop shortcut 		[x]"
cat << EOF >target/package/export/share/applications/org.winepak32.$NAME.desktop
[Desktop Entry]
Version=1.0
Name=$NICE_NAME
Exec=run.sh
Icon=org.winepak32.$NAME
StartupNotify=true
Terminal=false
Type=Application
Categories=Game;
...
if [ -e "$APP"/icon.png ]; then 
   cp "$APP"/icon.png \
   target/package/export/share/icons/hicolor/128x128/apps/org.winepak32.$NAME.png;
else
...
hicolor/128x128/apps/org.winepak32.$NAME.png;
...
chmod +x target/\[winepak32\]$DOT_NAME/install.sh
chmod +x target/\[winepak32\]$DOT_NAME/uninstall.sh
chmod +x target/\[winepak32\]$DOT_NAME/run.sh
cp "$0" target/package/files/make_winepak32.sh
