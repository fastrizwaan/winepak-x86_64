...
# https://github.com/fastrizwaan/winepak.x86_64
# copy the build-dir/files to wine32-6.0

#Output
echo "1.  Cleaning up previous target-$NAME		[x]"
...
mkdir -p target-$NAME/package/files/lib/i386-linux-gnu 
mkdir -p target-$NAME/package/files/lib/debug/lib/i386-linux-gnu
mkdir -p target-$NAME/package/files/lib32
mkdir -p target-$NAME/package/files/bin
mkdir -p target-$NAME/package/files/lib
mkdir -p target-$NAME/package/export/share/applications
mkdir -p target-$NAME/package/export/share/icons/hicolor/128x128/apps/
mkdir -p target-$NAME/\[winepak.x86_64\]$DOT_NAME

#Output
echo "3.  Creating run.sh script for sandbox	[x]"

cat << EOF > target-$NAME/package/files/bin/run.sh
#!/bin/bash
...
# on 1st run, copy TypingMasterPro to ~/.local/share/winepak.x86_64/$NAME/
elif [ ! -f ~/.local/share/winepak.x86_64/$NAME/1st-run-done.txt ];
     then
     cp -r /app/TypingMasterPro/ ~/.local/share/winepak.x86_64/$NAME/ && \
     touch ~/.local/share/winepak.x86_64/$NAME/1st-run-done.txt && \
     chmod a+rwx ~/.local/share/winepak.x86_64/$NAME/ -R
     cd ~/.local/share/winepak.x86_64/$NAME/ && \
     $WINEEXE "$EXE" \$@
...
cat << EOF >target-$NAME/package/metadata
[Application]
...
cat << EOF >target-$NAME/\[winepak.x86_64\]$DOT_NAME/install.sh
#!/bin/bash
...
cat << EOF >target-$NAME/\[winepak.x86_64\]$DOT_NAME/uninstall.sh
#!/bin/bash
...
cat << EOF >target-$NAME/\[winepak.x86_64\]$DOT_NAME/run.sh
#!/bin/bash
set -ex
...
cat << EOF >target-$NAME/package/export/share/applications/org.winepak.x86_64.$NAME.desktop
[Desktop Entry]
Version=1.0
Name=$NICE_NAME
Exec=run.sh
...
if [ -e "$APP"/icon.png ]; then 
   cp "$APP"/icon.png \
   target-$NAME/package/export/share/icons/hicolor/128x128/apps/org.winepak.x86_64.$NAME.png;
else
   ...
   cp hello-0.png target-$NAME/package/export/share/icons/\
...
chmod +x target-$NAME/\[winepak.x86_64\]$DOT_NAME/uninstall.sh
chmod +x target-$NAME/\[winepak.x86_64\]$DOT_NAME/run.sh
cp "$0" target-$NAME/package/files/make_winepak.x86_64.sh
echo "$0" "$ARGV" > target-$NAME/package/files/commandline.sh

echo "11. Building flatpak 			[x]"
echo "Started build-export"
flatpak build-export target-$NAME/repo target-$NAME/package && \
echo "Completed Build-export"
echo "Started bundle"
flatpak build-bundle --arch=$ARCH target-$NAME/repo  target-$NAME/\[winepak.x86_64\]$DOT_NAME/org.winepak.x86_64.$NAME.flatpak org.winepak.x86_64.$NAME

# removed runtime-repo for offline installation
#flatpak build-bundle --arch=$ARCH target-$NAME/repo  target-$NAME/\[winepak.x86_64\]$DOT_NAME/org.winepak.x86_64.$NAME.flatpak org.winepak.x86_64.$NAME --runtime-repo="https://flathub.org/repo/flathub.flatpakrepo"
